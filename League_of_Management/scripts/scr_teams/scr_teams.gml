    ...
    if (_index_to_replace != -1) {
		...
		if(_create_display){
			if(_team_struct.side == "Red"){
				create_player_display(_new_instance_id, global.playerXPositions[_index_to_replace + 5], global.playerYPositions[_index_to_replace + 5], true)
			}else{
				create_player_display(_new_instance_id, global.playerXPositions[_index_to_replace], global.playerYPositions[_index_to_replace], true)
			}
		}
    }
}